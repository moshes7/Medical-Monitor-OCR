...
if __name__ == '__main__':
    ...
    padding = 0.1  # amount of padding to add to each border of ROI
    nms_overlap_th = 0.4
    # grayscale_flag = True
    grayscale_flag = False
    blur_flag = False
    bilateral_filter_flag = False
    ...
    if blur_flag:
        image = blur(image, kernel_size=9)

    if bilateral_filter_flag:
    ...
    # find closest multiple of 32 for width and height
    width = int(np.floor(origW / 32) * 32)
    height = int(np.floor(origH / 32) * 32)
    ...
    boxes = non_max_suppression(np.array(rects), probs=confidences, overlapThresh=nms_overlap_th)
